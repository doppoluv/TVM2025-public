...
function collectUsedNames(node: any, names: Set<string>) {
    ...
    switch (node.type) {
        ...
        case 'unary':
            collectUsedNames(node.operand, names);
            break;
    }
...
export const getFunnyAst = {
    ...
    Conditional(ifKw, lp, cond, rp, then, elseKw, elseStmt) {
        return {type: 'if', condition: cond.parse(), then: then.parse(),
            else: elseStmt.numChildren > 0 ? elseStmt.children[0].parse() : null} as ast.ConditionalStmt;
    },
